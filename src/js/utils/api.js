...
export default {
    postMenu: (currentCategoryName, menuName) => {
        ...
        return fetch(url, {'method': 'POST', 'headers': {'Content-Type': 'application/json'}, 'body': JSON.stringify({'name' : menuName})})
            .then(data => {
                if (!data.ok) {
                    alert('400!');
                }
    ...
    updateMenu: (currentCategoryName, id, newMenuName) => {
        ...
        return fetch(url, {'method': 'PUT', 'headers': {'Content-Type': 'application/json'}, 'body': JSON.stringify({'name' : newMenuName})})
        .then(data => data.json());
    },
