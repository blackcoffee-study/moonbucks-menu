...
```
- [ ] 메뉴 추가
  - [ ] 메뉴 이름을 입력받고 확인 버튼을 누르면 메뉴가 추가한다. 
  - [ ] 메뉴 이름을 입력받고 엔터키를 누르면 메뉴가 추가한다. 
  - [ ] 추가되는 메뉴는 `<ul id="espresso-menu-list" class="mt-3 pl-0"></ul>` 최하단에 삽입한다.
  - [ ] 메뉴가 추가되면 input은 빈 값으로 초기화한다. 
  - [ ] 사용자 입력값이 빈 값이면 추가되지 않고 `alert` 창을 띄운다.
