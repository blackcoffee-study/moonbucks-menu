...
export default function compMenuItem(item, category) {
  ...
  const menuName = new Element('span', item.isSold ? Style.menuName.isSold : Style.menuName.default)
    .setElement()
    .setText(item.name)
    .getElement()
  ...
  const soldOutBtn = new Element('button', Style.soldOutBtn).setElement().setType('button').setText('품절').getElement()
