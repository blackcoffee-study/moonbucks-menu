...
```
- [x] 메뉴 추가
  - [x] 메뉴 이름을 입력받고 확인 버튼을 누르면 메뉴가 추가한다. 
  - [x] 메뉴 이름을 입력받고 엔터키를 누르면 메뉴가 추가한다. 
  - [x] 추가되는 메뉴는 `<ul id="espresso-menu-list" class="mt-3 pl-0"></ul>` 최하단에 삽입한다.
  - [x] 메뉴가 추가되면 input은 빈 값으로 초기화한다. 
  - [x] 사용자 입력값이 빈 값이면 추가되지 않고 `alert` 창을 띄운다.

- [x] 메뉴 수정
  - [x] 메뉴 수정 버튼을 누르면 `prompt` 창을 띄운다.
  - [x] 새로운 메뉴 이름을 입력받고 메뉴가 수정된다.

- [x] 메뉴 삭제
  - [x] 메뉴 삭제 버튼을 누르면 `confirm` 창을 띄운다.
  - [x] 메뉴가 삭제된다.

- [x] 총 메뉴 갯수
  - [x] 메뉴가 추가되면 총 메뉴 갯수가 증가한다.
  - [x] 메뉴가 삭제되면 총 메뉴 갯수가 증가한다.
```
...
- [ ] [localStorage](https://developer.mozilla.org/ko/docs/Web/API/Window/localStorage)에 데이터를 저장하여 새로고침해도 데이터가 남아있게 한다.
  ...
  - [x] localStorage 의 데이터를 불러온다.
  - [x] 메뉴 추가 시 localStorage 에 해당 메뉴 데이터를 추가한다.
  - [x] 메뉴 수정 시 localStorage 의 해당 메뉴 데이터를 수정한다.
  - [x] 메뉴 삭제 시 localStorage 의 해당 메뉴 데이터를 삭제한다.

- [ ] 에스프레소, 프라푸치노, 블렌디드, 티바나, 디저트 각각의 종류별로 메뉴판을 관리할 수 있게 만든다.
  - [x] 메뉴판 선택 시 카테고리 제목이 변경된다.
  - [x] 메뉴판 선택 시 localStorage 에서 해당 메뉴를 불러온다.
  - [x] 페이지에 최초로 접근할 때 localStorage 에서 espresso 메뉴를 불러온다.
  - [x] localStorage 에서 메뉴 데이터를 불러오면 화면에 출력한다.

- [ ] 품절 상태인 경우를 보여줄 수 있게, 품절 버튼을 추가하고 `sold-out` class를 추가하여 상태를 변경한다.
  - [x] 품절 버튼을 추가한다.
  - [x] 메뉴 품절 시 localStorage 의 해당 메뉴 데이터의 품절 정보를 수정한다.
  - [x] li 태그에 `sold-out` class를 추가 또는 삭제한다.
